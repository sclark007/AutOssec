...
module OssecCore
  def ossec_hostname_search
    ...
    node[:ossec][:rules].each do |_id, params|
      unless params.nil?
  ...
  def ossec_generate_agent_key(agent_hash)
    ...
    seed1 = rand(100_000_000_000).to_s
    seed2 = rand(100_000_000_000).to_s
  ...
  def ossec_agent_is_zombie?(id)
    if File.exist?("/var/ossec/bin/agent_control")
      ...
      if not status[6] || status[3] =~ /(Never connected|)/
        ...
        return true if three_days_ago > last_keep_alive
      end
    end
    false
  end
  ...
  def ossec_agent_should_be_removed?(id)
    if File.exist?("/var/ossec/bin/agent_control")
      ...
      if !status[6] || status[6] =~ /Unknown/
        ...
        return true if seven_days_ago > last_keep_alive
      end
    end
    false
  end
