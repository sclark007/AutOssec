...
ossec_agents.each do |agent|
  ...
  unless ossec_verify_agent(agent_hash, node)
  ...
  unless ossec_agent_has_valid_key?(agent_hash, node)
  ...
  unless ossec_agent_knows_key?(agent_hash, agent)
  ...
  unless ossec_agent_is_active?(agent_hash[:id])
...
node[:ossec][:agents].each do |agent_id, params|
  ...
  agent = ossec_agents.find { |n| (n[:ossec][:agents].key?(agent_id) && n[:ossec][:agent][:enable]) }

  next unless agent.nil?
